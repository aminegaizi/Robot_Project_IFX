...
/*
 * function ISR_ENCODER_L
 * Interrupt Service Routine for Left Encoder
 * Triggered by each rising or falling edge from the left encoder
 */
void ISR_ENCODER_L()
{
	interruptLeft_counter++;
	if (commande_movement == 1)//Going forward -> increment
	{positionLwheel++;}
	else if(commande_movement == 2)//Going backward -> decrement
	{positionLwheel--;}
}

/*
 * function ISR_ENCODER_R
 * Interrupt Service Routine for Right Encoder
 * Triggered by each rising or falling edge from the right encoder
 */
void ISR_ENCODER_R()
{
