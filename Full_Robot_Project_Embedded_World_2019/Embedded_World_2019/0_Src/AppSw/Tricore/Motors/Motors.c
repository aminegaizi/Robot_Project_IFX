...
extern volatile uint32 LeftEncoderCounter, RightEncoderCounter;

IfxStm_CompareConfig CompareCorrection;
Ifx_STM *stm2 = &MODULE_STM2;

volatile uint8 direction = 0; //1 = Forward, 2 = Backward
 ...
 */
 ...
 void Backward_1()
 {
	interruptRight_counter = 0;
	interruptLeft_counter = 0;
	Left_duty_cycle = 50;//35
	Right_duty_cycle = 50;//15
 ...
 void Right()
 {
	LeftEncoderCounter=0, RightEncoderCounter=0;
	interruptRight_counter = 0;
	interruptLeft_counter = 0;
 ...
 void Left()
 {
	...
	commande_movement = 4; //Flag for Turning Left
 }

 /*
  * Function used to set the H-Bridge and PWM pins for the Left wheel to go faster than the right wheel to get a curved trajectory
  */
 void F_RightCurb()
 {
	Left_duty_cycle = 45;
 ...
 /*
  * Function used to set the H-Bridge and PWM pins for the Right wheel to go faster than the Left wheel to get a curved trajectory
  */
 void F_LeftCurb()
 {
...
void B_RightCurb()
{
	...
	Left_duty_cycle = 45;//35
	Right_duty_cycle = 35;//15
...
void B_LeftCurb()
{
	Left_duty_cycle = 35;//35
	Right_duty_cycle = 45;//15
...
void STM_INTERRUPT_CORRECTION()
{
	...
	{
		Right_duty_cycle = 50; //Set Right Motor duty cycle to 50%
		Left_duty_cycle = Right_duty_cycle - (Forward_correction.stable * Encoders_Error); //Calculate Left Wheel Duty cycle depending on the Error between the encoders
	...
	if (commande_movement == 5 || commande_movement == 6) //Curv turning Right or Left
	{
		PWM_setDuty(Timers.PWM1_Bridge, Right_duty_cycle);
		PWM_setDuty(Timers.PWM2_Bridge, Left_duty_cycle);
	}

	if (commande_movement == 7 || commande_movement == 8) //Curv turning Right or Left
	{
		PWM_setDuty(Timers.PWM1_Bridge, Right_duty_cycle);
		PWM_setDuty(Timers.PWM2_Bridge, Left_duty_cycle);
	}
