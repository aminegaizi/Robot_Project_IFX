# 1 "0_Src/BaseSw/iLLD/TC27D/Tricore/Asclin/Std/IfxAsclin.c"
# 1 "C:\\Users\\Gaizi\\Desktop\\Robot_Project_IFX\\Robot_Project_TC27//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/BaseSw/iLLD/TC27D/Tricore/Asclin/Std/IfxAsclin.c"
# 29 "0_Src/BaseSw/iLLD/TC27D/Tricore/Asclin/Std/IfxAsclin.c"
...
# 30 "0_Src/BaseSw/iLLD/TC27D/Tricore/Asclin/Std/IfxAsclin.c" 2
...
{
    float32 frequency;
    IfxAsclin_ClockSource clockSource = IfxAsclin_getClockSource(asclin);
...
volatile Ifx_SRC_SRCR *const IfxAsclin_getSrcPointerEr(Ifx_ASCLIN *asclin)
{
    return (Ifx_SRC_SRCR *const)(&(*(Ifx_SRC*)0xF0038000u).ASCLIN.ASCLIN[IfxAsclin_getIndex(asclin)].ERR);
}


volatile Ifx_SRC_SRCR *const IfxAsclin_getSrcPointerRx(Ifx_ASCLIN *asclin)
{
    return (Ifx_SRC_SRCR *const)(&(*(Ifx_SRC*)0xF0038000u).ASCLIN.ASCLIN[IfxAsclin_getIndex(asclin)].RX);
}


volatile Ifx_SRC_SRCR *const IfxAsclin_getSrcPointerTx(Ifx_ASCLIN *asclin)
{
    return (Ifx_SRC_SRCR *const)(&(*(Ifx_SRC*)0xF0038000u).ASCLIN.ASCLIN[IfxAsclin_getIndex(asclin)].TX);
}
...
void IfxAsclin_setBaudrateBitFields(Ifx_ASCLIN *asclin, uint16 prescaler, uint16 numerator, uint16 denominator, IfxAsclin_OversamplingFactor oversampling)
{
    ...
    IfxAsclin_setPrescaler(asclin, prescaler);
    IfxAsclin_setNumerator(asclin, numerator);
    IfxAsclin_setDenominator(asclin, denominator);
    IfxAsclin_setOversampling(asclin, oversampling);
    IfxAsclin_setClockSource(asclin, clockSource);
}
...
boolean IfxAsclin_setBitTiming(Ifx_ASCLIN *asclin, float32 baudrate, IfxAsclin_OversamplingFactor oversampling, IfxAsclin_SamplePointPosition samplepoint, IfxAsclin_SamplesPerBit medianFilter)
{
    ...
    adder_facL_min = 0;
    f = (fpd * n) / d;
    relError = ( ((fOvs - f) < 0.0) ? -(fOvs - f) : (fOvs - f) );
    ...
    for (n = 2; ((!terminated) && ((n * d) <= 0xFFF)); n++)
    {
        ...
        {
            f = (fpd * n) / (n * d + count);
            newRelError = ( ((fOvs - f) < 0.0) ? -(fOvs - f) : (fOvs - f) );
