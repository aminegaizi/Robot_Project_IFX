# 1 "0_Src/BaseSw/iLLD/TC27D/Tricore/Vadc/Std/IfxVadc.c"
# 1 "C:\\Users\\Gaizi\\Desktop\\Robot_Project_IFX\\Robot_Project_TC27//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "0_Src/BaseSw/iLLD/TC27D/Tricore/Vadc/Std/IfxVadc.c"
# 29 "0_Src/BaseSw/iLLD/TC27D/Tricore/Vadc/Std/IfxVadc.c"
...
# 30 "0_Src/BaseSw/iLLD/TC27D/Tricore/Vadc/Std/IfxVadc.c" 2
...
void IfxVadc_enableGroupSync(Ifx_VADC *vadc, uint32 ccu6Num)
{
    ...
    if (ccu6Num == 0)
    {
        ...
        (*(volatile Ifx_CCU6_CLC*)0xF0002A00u).U = 0;

        if ((*(volatile Ifx_CCU6_CLC*)0xF0002A00u).U)
        {}

        (*(volatile Ifx_CCU6_T13PR*)0xF0002A54u).U = 4;
        (*(volatile Ifx_CCU6_CC63SR*)0xF0002A5Cu).U = 4;
        (*(volatile Ifx_CCU6_MODCTR*)0xF0002A80u).B.ECT13O = 1;
        (*(volatile Ifx_CCU6_TCTR4*)0xF0002A78u).U = (1 << (14u)) | (1 << (9u));
        (*(volatile Ifx_CCU6_MOSEL*)0xF0002A0Cu).B.TRIG1SEL = 0;
    }
    else if (ccu6Num == 1)
    {

        (*(volatile Ifx_CCU6_CLC*)0xF0002A00u).U = 0;
        (*(volatile Ifx_CCU6_CLC*)0xF0002B00u).U = 0;

        if ((*(volatile Ifx_CCU6_CLC*)0xF0002B00u).U)
        {}

        (*(volatile Ifx_CCU6_T13PR*)0xF0002B54u).U = 4;
        (*(volatile Ifx_CCU6_CC63SR*)0xF0002B5Cu).U = 4;
        (*(volatile Ifx_CCU6_MODCTR*)0xF0002B80u).B.ECT13O = 1;
        (*(volatile Ifx_CCU6_TCTR4*)0xF0002B78u).U = (1 << (14u)) | (1 << (9u));
        (*(volatile Ifx_CCU6_MOSEL*)0xF0002A0Cu).B.TRIG1SEL = 1;
    }
...
Ifx_VADC_G_RES IfxVadc_getResultBasedOnRequestSource(Ifx_VADC *vadc, Ifx_VADC_G *group, IfxVadc_ChannelId channel, IfxVadc_RequestSource sourceType)
{
    ...
    else
    {
        if ((sourceType == IfxVadc_RequestSource_background) && (group->CHCTR[channel].B.RESTBS == 1))
        {
            ...
            tmpResult.B.DRC = vadc->GLOBRES.B.GNR;
...
volatile Ifx_SRC_SRCR *IfxVadc_getSrcAddress(IfxVadc_GroupId group, IfxVadc_SrcNr index)
{
    ...
    if (IfxVadc_SrcNr_shared0 <= index)
    {
        ...
        if ((group & 0x1) != 0)
        {
            group = IfxVadc_GroupId_global1;
        }
        else
        {
            group = IfxVadc_GroupId_global0;
        }
    }
    else
    {
...
uint32 IfxVadc_initializeFAdcI(Ifx_VADC *vadc, uint32 fAdcI)
{
    ...
    if (!((result >= (500000)) && (result <= (20000000))))
    {
        result = 0;
    }
    else
...
void IfxVadc_resetKernel(Ifx_VADC *vadc)
{
    ...
    while (vadc->KRST0.B.RSTSTAT == 0)

    {}

    IfxScuWdt_clearCpuEndinit(passwd);
    vadc->KRSTCLR.B.CLR = 1;
    IfxVadc_disableAccess(vadc, IfxVadc_Protection_globalConfig);
    IfxScuWdt_setCpuEndinit(passwd);
}
...
void IfxVadc_setArbiterPriority(Ifx_VADC_G *vadcG, boolean slotEnable, IfxVadc_RequestSlotPriority prio, IfxVadc_RequestSlotStartMode mode, IfxVadc_RequestSource slot)
{
    if (slotEnable != 0)
    {
        vadcG->ARBPR.U |= slotEnable << ((24u) + slot);
        vadcG->ARBPR.U &= ~((0x3u) << (slot * 4u));
        vadcG->ARBPR.U |= (prio << (slot * 4u));

        if (mode != IfxVadc_RequestSlotStartMode_waitForStart)
        {
            vadcG->ARBPR.U |= 0x1u << ((3u) + (slot * 4u));
        }
        else
        {
            vadcG->ARBPR.U &= ~(0x1u << ((3u) + (slot * 4u)));
...
void IfxVadc_startupCalibration(Ifx_VADC *vadc)
{
    ...
    do
    {
        ...
        for (adcCalGroupNum = 0; adcCalGroupNum < (8); adcCalGroupNum++)
        {
            if (IfxVadc_getAdcCalibrationActiveState(vadc, adcCalGroupNum) != 0)
            {
                calibrationRunning = 1;
            }
            else
            {
