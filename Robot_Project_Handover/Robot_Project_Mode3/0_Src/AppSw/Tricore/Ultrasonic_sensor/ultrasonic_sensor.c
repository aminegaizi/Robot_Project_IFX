...
void configUltrasonicSensor()
{
	...
	IfxSrc_init(&(SRC_SCU_SCU_ERU3),0,PRIORITY_ISR_ULTRASONIC); //SRC_SCU_SCU looks as an error, but it is not one, it's actually a bug from eclipse
	IfxSrc_enable(&(SRC_SCU_SCU_ERU3)); //To know if it is an error from the code or from eclipse struggling to link variables, look at the "console" tab, if an .elf file is created
	//it means that the compilation went well. Do not pay too much attention to the Problems tab.
}

/*
 * void sendTrig(IfxPort_Pin port_pin_output)
 * create a 10us high state signal on the port_pin pin
 * Used for the config of the ultrasonic sensor
 */
void sendTrig(IfxPort_Pin port_pin_output)
{
