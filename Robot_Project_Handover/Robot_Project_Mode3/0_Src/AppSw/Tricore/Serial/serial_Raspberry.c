...
IFX_INTERRUPT(serialTxISR_Raspberry, 0, ISR_PRIORITY_SERIAL_TX);//interrupt transmission
IFX_INTERRUPT(serialRxISR_Raspberry, 0, ISR_PRIORITY_SERIAL_RX);//interrupt reception
IFX_INTERRUPT(serialErISR_Raspberry, 0, ISR_PRIORITY_SERIAL_ER);//interrupt error
 ...
 * This serial config allows us to communicate through UART with the raspberry
 */
void serial_config_Raspberry() {
	...
	ascConfig.baudrate.baudrate = SERIAL_BAUDRATE_RASP; // FDR values will be calculated in initModule
