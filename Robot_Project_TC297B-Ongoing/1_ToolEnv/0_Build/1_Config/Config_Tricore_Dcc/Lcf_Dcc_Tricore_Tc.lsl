...
MEMORY
{
	dsram2: org = 0x50000000 + 100, len = 240K - 100 /* Workaround for segment overlap problem*/
	psram2: org = 0x50100000, len = 32K
	
	dsram1: org = 0x60000000 + 100, len = 240K - 100 /* Workaround for segment overlap problem*/
	psram1: org = 0x60100000, len = 32K
	
	dsram0: org = 0x70000000 + 100, len = 120K - 100 /* Workaround for segment overlap problem*/
	psram0: org = 0x70100000, len = 32K
	
	psram_local: org = 0xc0000000, len = 32K
...
/*	} > dsram2 */
	...
	GROUP :
	{
		.psram2_text (TEXT) LOAD(> pfls0) COPYTABLE :
		{
			. = ALIGN(2);
			*(.psram_cpu2)
			*(.cpu2_psram)
	...
	GROUP :
	{
		.psram1_text (TEXT) LOAD(> pfls0) COPYTABLE :
		{
			. = ALIGN(2);
			*(.psram_cpu1)
			*(.cpu1_psram)
